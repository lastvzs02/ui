...
def callback_worker(call):
    try:
        ...
        if call.data == "video":
            bot.edit_message_text("Выберите рвзрешение", call.message.chat.id, call.message.message_id,
                                  reply_markup=choose_resolution(call.message, some_data[2]))
            db_conn.set_content_type(call.message.chat.id, 'video')

        elif call.data == "audio":
            wait_mess = bot.edit_message_text("Подождите...", call.message.chat.id, call.message.message_id)
